...
rePreProcessData = true;

%% Load h5 files and process each person's joint positions from camera and IMU orientation
for iSubj = [1]% 1:5
	folderPrefix = './s1_acting';% [DATA_FOLDER '/s' num2str(iSubj)];  % Folder where all the data for subject s_i is
	...
	for iActivity = 1:length(activities)
		...
		if exist(activityIMUfile, 'file')~=2  % Make sure folder exists (folder=7) as well as IMU data (file=2)
		...
		for iCam = 1:8
			...
			activityCamFile = [folderPrefix '/TC_S' num2str(iSubj) '_' activity '_cam' num2str(iCam) '.h5'];
...
for iWin = 1:length(windowLengths)
	...
	for iSubj = [1]% 1:5
		...
		for iActivity = 1:length(activities)
			...
			for iCam = 1:length(dataPreProc.dataCams)
				...
				while true
					...
					tInds = (tS <= t) & (t < tE);
					for iMethod = 1%:3
						[processed(iWin).scores{iCam,iMethod}{iSegment}, bodyPartToJointsAssoc] = computeSimilarityMatrix(dataPreProc.dataIMU, dataPreProc.dataCams(iCam), 1:length(dataPreProc.dataCams(iCam).camPos), [], iMethod, [], [], tInds);
					end
...
for iActivity = 1%:length(activities)
	activity = activities{iActivity};
	for iSubj = 1%:5
		activityProcessedFile = [DATA_FOLDER '/' activity '_processed.mat'];
...
for iWin = 1:length(windowLengths)
	for iSubj = [1]%1:5
        folderPrefix = 's1_acting';%[DATA_FOLDER '/s' num2str(iSubj)];
		%w(end+1) = parfeval(@processPreProcessedData, 0, activities, iWin, methods, cams, folderPrefix, tWindowOverlap, bodyPartToJointsAssoc);
		w(end+1) = processPreProcessedData(activities, iWin, methods, cams, folderPrefix, tWindowOverlap,10);
	end
end
